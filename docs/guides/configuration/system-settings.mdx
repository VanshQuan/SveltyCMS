...
Settings are organized into groups, including:

| Tab                          | Icon | Description                                      | Status    |
| ---------------------------- | ---- | ------------------------------------------------ | --------- |
| **General Settings**         | ⚙️   | Site identity, localization, maintenance mode    | ✅ Active |
| **Languages & Localization** | 🌐   | System language (admin UI) and content languages | ✅ Active |
...
Open **Config → System Settings → Languages & Localization** (`/config/system-settings?group=languages`). This is the same dual-language model as Setup: **system language** (admin chrome) on the right, **content language** on the left.

**English and German ship compiled.** You can add any ISO 639-1 code as a system language after install — including RTL (`ar`, `he`, `fa`, `ur`, …). Layout direction uses `getTextDirection()` in `@utils/string`.

### Change or extend the admin UI language

1. Set **Default System Language** (`BASE_LOCALE`) to the default for new sessions.
2. Click **Add** on **System Languages** (`LOCALES`) and search the ISO list (same chip picker as content languages). RTL codes show an **RTL** badge; bundled catalogs show **Translated**.
3. **Save**. The CMS:
   - Persists `LOCALES` / `BASE_LOCALE` in the database
   - Writes `project.inlang/settings.json`
   - Creates `src/messages/{locale}.json` if missing
   - Runs `bun translate` then `bun run paraglide`
4. Switch language from the sidebar. Leave the dev server running so Vite picks up `src/paraglide/`. If strings stay English, run `bun translate` and `bun run paraglide` yourself.

New locales from this path are **machine-translated drafts** (or English fallback). Review `src/messages/{locale}.json` in Fink or a PR before treating them as production copy — unlike vendor packs that ship community-reviewed strings. See [System Languages](./system-languages.mdx).

Content languages (`DEFAULT_CONTENT_LANGUAGE` / `AVAILABLE_CONTENT_LANGUAGES`) stay independent — they do not require a rebuild.

Full catalog workflow (Fink / GitHub PRs): [System Languages](./system-languages.mdx).
...
**For Advanced Usage:** See [Import/Export System](/docs/reference/database/index.mdx) for API documentation, encryption details, and automation workflows.
...
- [Import/Export System (Dev Guide)](/docs/reference/database/index.mdx) - API usage, workflows, encryption
- [Cache System](/docs/reference/architecture/cache-system) - Detailed cache configuration
- [API Reference](/docs/reference/api/index.mdx) - API endpoints
- [Authentication](/docs/reference/api/auth.mdx) - User authentication
