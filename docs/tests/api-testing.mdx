...
Unit-level dispatcher checks use thin shared helpers (`invokeApi` in `tests/unit/utils/mock-event.ts`) — not a separate HTTP-style client. See [API Unit Helpers](./api-unit-helpers.mdx).
...
1. **Config Isolation**: Local integration/E2E/hooks use **`config/private.test.ts` only**. They must never read or write the developer's **`config/private.ts`** (live CMS DB — e.g. `sveltycms.db`). See `src/utils/private-config-policy.ts`.
2. **Ephemeral Mirror Only**: CI may create a short-lived `config/private.ts` on the runner as a mirror of the test config — never committed or pushed.
...
- **Collection mutations**: `tests/integration/api/collections-mutations.test.ts` — create → PATCH → schedule → bulk publish → clone → bulk delete, with a **count-must-decrease** assertion so “delete created a row” cannot go green. Unit dispatcher: `tests/unit/api/collections.test.ts` (`POST /batch` delete/status/clone/schedule). Adapter `insertMany`/`updateMany`/`deleteMany` alone does **not** cover the HTTP batch router.
...
2. **Testing API (`/api/testing`)**: Handles state reset, seed, and isolated fixture orchestration for black-box E2E and integration tests under `TEST_MODE=true`. It is not exposed to production and is never used for production benchmarks or compliance verification (which target real routes like `POST /api/gdpr`).
...
# Full matrix parity (Docker profile up; slower — adapters/auth PRs)
...
- `tests/integration/api/user-extended.test.ts` → `POST /api/user/batch` (block/unblock/delete + negatives)

Keep bulk **HTTP** coverage there; use unit tests for pure batch helpers only.

### Session → page load (P0, 2026-07-18)

`tests/integration/api/session-page-load.test.ts` proves a login cookie unlocks SvelteKit `__data.json` for admin pages (`/user`, `/dashboard`, `/config/collectionbuilder`, …) — not only `/api/user`.
Unit companions: `tests/unit/auth/sessions/session-cookies.test.ts`, `tests/unit/api/testing-login-cookie-contract.test.ts`.
See [Headless-First Test Inventory](./headless-test-inventory.mdx).
