...
## 9. GDPR Compliance API (`/api/gdpr`)

The GDPR compliance endpoint implements regulatory data portability (Article 20) and Right to Erasure / Right to be Forgotten (Article 17). It is available as an authenticated endpoint for self-service or for administrators.

### `POST /api/gdpr`

| Action | Payload | Description |
| :--- | :--- | :--- |
| `export` | `{ "action": "export", "userId": "<id>" }` | **Article 20 Data Portability**: Exports user profile metadata and audit trail history. |
| `anonymize` | `{ "action": "anonymize", "userId": "<id>", "reason": "..." }` | **Article 17 Soft Erasure**: Replaces PII with pseudonymized ghost identities while maintaining schema foreign key integrity. |
| `erase` / `wipe` | `{ "action": "erase", "userId": "<id>", "reason": "..." }` | **Article 17 Deep Wipe**: Permanently erases user identity and cascades across active sessions, tokens, and audit logs with 0 orphans. |
...
- [ADR: Testing 2026](../../tests/adr-testing-2026.mdx)
````
