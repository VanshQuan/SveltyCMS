...
### Dual-Layer Strategy & Distributed Multi-Node Cluster

```
Layer 1: In-Memory LRU Cache → <0.001ms (L1 hit, zero allocation)
Layer 2: Redis (Optional)     → <0.5ms (distributed cluster, edge sync, cross-node invalidation)
Layer 3: Database (Persistent) → PostgreSQL, MariaDB, MongoDB, SQLite (0.042-3.261ms)
```

```mermaid
flowchart TD
    subgraph Clients ["Traffic / Web / Mobile"]
        C1[Client 1]
        C2[Client 2]
    end

    subgraph Cluster ["SveltyCMS Multi-Node Cluster"]
        subgraph NodeA ["SveltyCMS Instance A"]
            L1A["L1 RAM LRU (0.001ms)<br>Zero-Copy Buffers"]
            SM_A["Route State Machine<br>AI Speculative Prewarm"]
        end
        subgraph NodeB ["SveltyCMS Instance B"]
            L1B["L1 RAM LRU (0.001ms)<br>Zero-Copy Buffers"]
            SM_B["Route State Machine<br>AI Speculative Prewarm"]
        end
    end

    subgraph DistributedState ["Distributed Redis L2 Layer"]
        L2[("Redis Sentinel / Cluster <br> APQ Registry · L2 Response Cache")]
        PUBSUB{{"Redis Pub/Sub <br> svelty:cache:invalidation"}}
    end

    subgraph Persistence ["Enterprise Database Cluster"]
        DB[("PostgreSQL / MariaDB / MongoDB / SQLite")]
    end

    C1 --> NodeA
    C2 --> NodeB

    NodeA <-->|L1 Miss / Promotion| L2
    NodeB <-->|L1 Miss / Promotion| L2

    NodeA -.->|Mutation Invalidate| PUBSUB
    PUBSUB -.->|Real-time Invalidation Push| L1B
    PUBSUB -.->|Real-time Invalidation Push| L1A

    NodeA ===|Write / DB Fallback| DB
    NodeB ===|Write / DB Fallback| DB
```
...
flowchart TD
    ...
    SF -->|no| DB[(Database: Postgres / MariaDB / Mongo / SQLite)]
...
### Next-Gen 2026 Cache Features

#### 1. GraphQL Automatic Persisted Queries (APQ)

SveltyCMS natively implements the **Apollo APQ open standard protocol** (`extensions: { persistedQuery: { sha256Hash: "<hash>" } }` & `?extensions=...` / `?hash=...`):

- **Bandwidth Reduction**: Clients send compact 64-bit/SHA-256 hashes instead of multi-kilobyte GraphQL query documents.
- **Zero-Parse AST Hits**: Queries registered in the `cacheService` APQ registry (`apq:${hash}`) bypass Yoga query parsing and AST construction entirely.
- **Fail-Open Handshake**: If an APQ hash is not yet registered, SveltyCMS returns HTTP 200 with standard `PERSISTED_QUERY_NOT_FOUND`, allowing the client to automatically retry and register the full query.

#### 2. AI-Driven Speculative Pre-Warming

The **`routeResourceStateMachine`** connects directly with the **Behavioral Learning Engine** (`predictNextPath`):

- When users navigate the CMS, transition probability heatmaps track subsequent navigation patterns.
- `speculativePrewarm()` selects the highest-scoring Markov transition from the decaying heat map and triggers non-blocking, single-flight background pre-warming of target endpoints before the user even hovers over the next link.
- **Zero-Wait Navigation**: Response caches are primed in RAM before click events occur.

#### 3. Pre-Compressed Binary Byte Serving (Zero-CPU)

- For API and GraphQL responses $>1\text{ KB}$, `responseCache` asynchronously computes and caches pre-compressed **Brotli** and **Gzip** binary chunks (`Uint8Array`).
- On `TURBO-HIT` and GraphQL cache hits, matching `Accept-Encoding` requests receive pre-compressed byte arrays directly, bypassing on-the-fly CPU compression during traffic spikes.

### Key: L1 is In-Memory, L2 is Redis

The L1 cache is a local `LRUCache` instance (not Redis). Redis is the optional L2 layer for distributed deployments. This means even without Redis, the cache system is fully operational with sub-µs L1 hits. In enterprise multi-node deployments with PostgreSQL, MariaDB, or MongoDB, Redis acts as the cluster synchronizer and persistent L2 layer.

#### L1 Memory Containment & Bounded Footprint

- **Byte Budget (`maxSize`)**: Defaults to **128 MB** (configurable via `CACHE_L1_MAX_BYTES`). Prevents unbounded heap growth regardless of total entry count.
- **Entry Cap (`max`)**: Defaults to **200,000** (configurable via `CACHE_L1_MAX_ENTRIES`).
- **Single-Entry Ceiling (`maxEntrySize`)**: Defaults to **16 MB** (configurable via `CACHE_L1_MAX_ENTRY_BYTES`), preventing oversized individual payloads from evicting hot cache entries.
- **$O(1)$ Size Estimator**: Accurately sizes pre-stringified response bodies and employs a flat overhead estimate for structured objects without recursive traversal overhead.
...
### Distributed Rate Limiter & Token-Bucket L2 Integration

The rate limiting middleware (`src/hooks/handle-rate-limit.ts`) integrates with the Redis L2 layer to coordinate request throttling across clustered instances:

- **Atomic Lua Engine**: Redis L2 executes an atomic Lua script that computes continuous token replenishment (`refillRatePerMs = capacity / windowMs`) and decrements available tokens in a single network round-trip.
- **Fail-Safe Circuit-Breaker Fallback**: Network queries to Redis L2 are bounded by a strict **15 ms timeout**. If Redis is unreachable or exceeds latency limits, an internal circuit breaker trips with a **5-second backoff**, instantly diverting throttling decisions to process-local in-memory Token Buckets (`< 2 µs`). This ensures cluster availability without dropping requests or increasing user-perceived latency.
- **Zero-Allocation Session Cache Peeking**: Privilege-tier multipliers (Admin $10\times$, Authenticated $2\times$, Anonymous $1\times$) are evaluated synchronously against the session cache (`peekSessionFromCacheSync`) in `handle-authentication.ts` without triggering async microtask ticks.

### Field-Level Encryption & Ciphertext Cache Isolation

In accordance with our zero-trust data protection model (`docs/reference/security/field-level-encryption.mdx`):

- **Ciphertext Storage**: L1 RAM and L2 Redis caches store encrypted envelopes (`v1:iv:tag:ciphertext`) for all fields flagged with `encrypt: true`. Unencrypted plaintext is **never** persisted in shared cache infrastructure.
- **Read-Path Decryption Cloning**: Upon a cache hit, the read pipeline decrypts field values into an isolated shallow clone of the document before returning it to the SDK/API caller, preventing plaintext leaks back into the cache layer.
