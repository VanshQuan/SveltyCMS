---
path: "docs/reference/architecture/language-architecture.mdx"
...
**Available Languages (default ship):** `en`, `de`
...
Setup and System Settings can add any ISO 639-1 code. New locales are appended here, then machine-translated. See [System Languages guide](../../guides/configuration/system-languages.mdx).
...
### Architecture {#system-language-arch}
...
```
src/messages/en.json          # English source catalog (edit this)
src/messages/de.json          # German (bundled)
src/messages/{locale}.json    # Extra locales (machine-translated or community)
...
// src/messages/en.json
{
  ...
  "applayout_systemlanguage": "System Language"
}
```
...
**Preferred:** add the ISO code in Setup → System Languages, or later in **Config → System Settings → Languages & Localization** (`LOCALES` / `BASE_LOCALE`). Saving writes `project.inlang/settings.json` and runs `bun translate` then `bun run paraglide`.

**Manual / community:**

1. Add the code to `project.inlang/settings.json` `locales` and `languageTags`.
2. `bun translate` — Inlang machine-fills `src/messages/{locale}.json`.
3. `bun run paraglide` — compile type-safe messages.
4. Review the catalog and open a GitHub PR, or edit in [Fink](https://fink.inlang.com/github.com/SveltyCMS/SveltyCMS).

The language switcher reads configured `LOCALES` plus bundled Paraglide locales. RTL uses `getTextDirection()` from `@utils/string` (not a hardcoded `<select>`). Until a catalog is compiled, the UI stays in English and still sets `html lang` + `dir`.
...
**Admin Interface:** `/config/system-settings` → Languages & Localization tab
...
<Callout type="info">
  Unlike system languages, content languages can be added **without rebuilding** the application.
</Callout>
...
<Callout type="error">
  ⚠️ **WARNING:** Removing a language from `AVAILABLE_CONTENT_LANGUAGES` can cause **permanent data
  loss**!
</Callout>
  ...
  for (const collection of collections) {
    ...
    for (const entry of entries) {
      ...
      const mainData = Object.fromEntries(
        Object.entries(entry).filter(([key, value]) => typeof value !== "object" || value === null),
      );

      await sqlDb.insert(collection.id).values(mainData);

      // Insert translations (translated fields)
      const translatedfields = Object.entries(entry).filter(
    ...
    for (const [fieldName, translations] of Object.entries(translatedfields)) {
      for (const [lang, value] of Object.entries(translations as any)) {
        await this.db
          ...
          .onConflictDoUpdate({
            target: [
              getTranslationTable(collection).entity_id,
              getTranslationTable(collection).field_name,
              getTranslationTable(collection).language,
            ],
            set: { value },
          });
      }
...
<input value={safeValue} oninput={(e) => handleInput(e.currentTarget.value)} placeholder={field.placeholder} class="w-full rounded border border-surface-300 px-3 py-2" />

<!-- Character counter (no translation needed) -->
{#if field.count}
	<Badge size="sm">{count}/{field.maxLength}</Badge>
{/if}
```
...
## 🚀 Why SveltyCMS Localization is Superior

SveltyCMS 2026 goes beyond basic i18n to offer **Agentic Localization**. This system understands the cultural and regulatory context of your target market.

### 1. Agentic AI Co-Pilot

While competitors rely on static integrations with external translation tools, SveltyCMS embeds an **AI Translation Brain** directly into the entry editor.

- **Cultural Context Awareness**: The AI doesn't just swap words; it adapts idioms and tone based on the target region's cultural nuances.
- **Brand Consistency**: The AI is trained on your existing content via the **Knowledge Core (MCP)**, ensuring the German version of your "About" page sounds exactly like your English one.
- **One-Click Localize**: Instantly translate entire entries while maintaining complex JSON structures (e.g., in Repeater fields).

### 2. Regional Logic & Overrides

SveltyCMS supports **Structural Localization**. This means a "Post" entry in the US might have a different structure than the same entry in the EU.

- **Regulatory Fields**: Use the `RegionalOverride` pattern to automatically inject mandatory GDPR disclosures for EU locales while hiding them for the US.
- **Dynamic Pricing**: Link pricing models to locales at the field level, allowing "19.99 USD" to become "18,50 EUR" with proper regional formatting and tax inclusion.
- **Visual Context**: Translators can work directly in the **Live Preview**, clicking on a website component to instantly jump to the corresponding translation field.

### 3. Integrated Lix/Inlang Workflow

We utilize **Paraglide-JS** and the **Inlang Ecosystem** for high-performance, type-safe translations.

- **Type-Safety**: Your frontend code (Svelte 5) receives full TypeScript definitions for all translated strings, preventing "Missing Translation" errors at compile time.
- **Lix Versioning**: Every translation change is tracked as a "Change Request" in the Lix versioning system, allowing for professional audit trails.

---

## 🛠 Developer Blueprint: Regional Logic

To implement a locale-specific field, use the following pattern in your collection schema:
...
export const schema: Schema = {
  ...
  fields: [
    ...
    {
      db_fieldName: "eu_tax_info",
      widget: widgets.Textarea,
      label: "EU Regulatory Info",
      // Logic: Only show this field if the editor is working in an EU locale
      display: {
        condition: (data, locale) => ["de-DE", "fr-FR", "it-IT"].includes(locale),
      },
    },
  ],
};
...
This ensures the UI remains clean for US editors while being functionally complete for EU legal compliance.
...
- **[Database Methods](../database/database-methods.mdx)** - Multilingual data handling
- **[System Settings](./system-settings-architecture.mdx)** - Dynamic language configuration
- **[Widget Development](../../development/widgets/index.mdx)** - Creating multilingual widgets

---

_Last Updated: March 17, 2026_
_Architecture Version: 4.0_
