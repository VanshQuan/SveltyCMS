...
### Zero-Egress Privacy & Application-Layer Encryption (ALE)

- **Zero Data Egress**: Client-side inference executes models locally inside the operator's browser session. In contrast to SaaS-dependent AI integrations, no image buffers, prompt contexts, or generated outputs are transmitted to external AI providers.
- **Field-Level Encryption Alignment**: Sensitive content fields designated with `encrypt: true` (AES-256-GCM at rest, see [Field-Level Encryption](../security/field-level-encryption.mdx)) remain strictly protected: client-side processing bypasses WAN transmission altogether, preventing third-party model ingestion or PII leakage into third-party training corpuses.
- **Fail-Safe Offline Resilience**: When disconnected from external networks, the client runtime operates entirely offline via cached WebAssembly binaries and `.tflite` model weights.
