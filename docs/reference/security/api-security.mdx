---
path: "docs/reference/security/api-security.mdx"
...
### 4.3 Batch Operation Guard & Upper-Bound Limits

To prevent memory exhaustion and DoS attacks through oversized payloads:

- **Mixed-Tenant Rejection**: `batch-module.ts` rejects coalesced operations containing mixed tenant IDs — preventing cross-tenant data leakage in shared-DB deployments.
- **Batch Size Ceiling**: All batch endpoints (`/api/tokens/batch`, `/api/users/batch`, `/api/media/download`, `/api/system/order`) enforce an upper-bound cap (`ids.length <= 100`). Exceeding this limit returns an immediate 400 validation error.
- **Single-Query Batch Operations**: Batch token deletion uses single round-trip database queries (`DELETE ... WHERE id IN (...)` / `deleteMany`), eliminating N+1 sequential database round-trips.
- **AST Scanner Guard (`batch-size-unbounded`)**: Static security analysis in `scripts/scan-security-risk.ts` scans all API handlers using the TypeScript compiler AST to ensure every batch block enforces a length cap assertion before processing.
...
| **API rate limit**       | `handle-rate-limit.ts`     | Hardware-aware Token-Bucket (Redis L2 primary + local memory fallback); **commerce lane** isolated (`X-RateLimit-Lane`) |
...
| **Retry-After header**   | RFC 6585 compliant         | All 429 responses include `Retry-After: 60` and `X-RateLimit-*` headers                                                 |
...
### 7.3 Role-Based Multi-Factor Authentication Enforcement (MFA-RBAC)

SveltyCMS enforces mandatory multi-factor authentication on a per-role basis:

- **Per-Role Mandate (`Role.mfaRequired`)**: When `mfaRequired: true`, users assigned to this role must complete interactive 2FA enrollment and verification before accessing protected system resources.
- **Session AMR Tracking (RFC 8176)**: Sessions capture Authentication Methods References:
  - `amr: ["pwd"]` — Password authentication only.
  - `amr: ["pwd", "mfa"]` — Interactive 2FA verified (TOTP, hardware passkey/WebAuthn, backup code).
  - `mfaVerifiedAt` — ISO timestamp recording when second-factor elevation occurred.
- **Endpoint Gate Enforcement**: In `_checkEndpointPermission`, requests made under a mandatory-MFA role that carry non-MFA sessions (`amr: ["pwd"]` or `amr: ["pwd", "trusted_device"]`) are immediately rejected with HTTP 403 `MFA_REQUIRED`.
- **Exempt Routes**: Self-service 2FA setup/verification (`/api/auth/2fa/*`) and session teardown (`/api/auth/logout`) remain accessible so users can authenticate their factor or safely log out.
- **Trusted-Device Override**: Device-level trust (`__Host-2fa-trusted-device`) is unconditionally disabled for any user assigned to a role with `mfaRequired: true`. Interactive second factor is required on every login.

---

### 7.4 Hardware-Aware Token-Bucket Rate Limiter & Throttling

To eliminate the 2× traffic spike artifacts inherent in fixed-window rate limiters, SveltyCMS implements a continuous Token-Bucket rate limiting engine in `src/hooks/handle-rate-limit.ts`:

```mermaid
flowchart TD
  REQ[Incoming HTTP Request] --> SYNC_AUTH[Sync Session Peeker]
  SYNC_AUTH --> TIER{User Privilege Tier}
  TIER -->|Admin| CAP_ADMIN[Capacity Multiplier: 10x]
  TIER -->|Authenticated| CAP_USER[Capacity Multiplier: 2x]
  TIER -->|Anonymous| CAP_ANON[Capacity Multiplier: 1x]

  CAP_ADMIN & CAP_USER & CAP_ANON --> HW{SystemMonitor CPU/Lag}
  HW --> COST[Adjusted Token Cost]

  COST --> REDIS_CHECK{Redis Active & Circuit Closed?}
  REDIS_CHECK -->|Yes| REDIS_LUA[Atomic Lua Token-Bucket Script <15ms]
  REDIS_CHECK -->|No / Timeout / Circuit Open| MEM_FALLBACK[Local In-Memory Token Bucket <2µs]

  REDIS_LUA & MEM_FALLBACK --> EVAL{Tokens Available?}
  EVAL -->|Yes| ALLOW[Allow Request + Append X-RateLimit Headers]
  EVAL -->|No| REJECT[429 Too Many Requests + Retry-After]
```

1. **Continuous Token Replenishment**: Rather than resetting counters at minute boundaries, tokens are replenished continuously based on elapsed milliseconds (`refillRatePerMs = capacity / windowMs`). Requests consume tokens based on adjusted cost.
2. **Distributed Redis L2 Primary**: Multi-node deployments execute an atomic Lua script directly on Redis:
   - Computes dynamic replenishment and token decrement in a single network round-trip.
   - Sets key TTL automatically matching the window duration.
3. **Resilient Circuit-Breaker Fallback**:
   - Redis commands are constrained by a strict **15 ms timeout**.
   - On Redis connection failure, timeout, or error, an internal circuit breaker trips with a **5-second backoff**.
   - Requests immediately fall back to a high-speed, local in-memory Token Bucket (`< 2 µs`), ensuring zero downtime and zero failed requests during Redis outages.
4. **Adaptive User-Tier Quotas**:
   - **Administrators**: $10\times$ capacity multiplier to guarantee unblocked CMS operations during batch management.
   - **Authenticated Users**: $2\times$ capacity multiplier for verified editors and interactive staff.
   - **Anonymous / Public**: $1\times$ baseline quota protecting against unauthenticated scrapers and brute-force attempts.
   - Evaluated via a **zero-overhead synchronous session cache peeker** (`peekSessionFromCacheSync`) in `handle-authentication.ts` avoiding circular dependencies and Promise overhead.
5. **Hardware-Aware Load Shedding**:
   - Integrates with `SystemMonitor` to measure event loop lag and CPU load.
   - Under heavy system pressure, token cost scales between $0.8\times$ (idle system) up to $2.0\times$ (saturated host), actively dampening traffic before server responsiveness degrades.
...
- **Incident response**: [Full runbook](./incident-response.mdx)
...
- [Incident Response](./incident-response.mdx)
- [Login Security](./login-security.mdx)
- [Secrets & Tokens Inventory](./secrets-inventory.mdx)
