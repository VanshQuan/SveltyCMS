...
- **Centralized Build Orchestrator & Scripts Consolidation (September 2026)**:
  - **TypeScript Build Orchestration (`scripts/build.ts`)**: Programmatic builder utilizing the native `esbuild` JavaScript API to bundle `src/collaboration/yjs-sync-server.ts` and sync `module-worker.server.ts`, replacing 9 fragile, duplicated ~250-character inline shell strings in `package.json`.
  - **Scanner-Enforced Security**: Eliminates `shell: true` execution in build scripts, strictly adhering to `scripts/scan-security-risk.ts` process safety rules.
  - **Lean CLI Surface**: Consolidated fragmented `config:*` sub-commands into `"config": "bun run scripts/config-cli.ts"` and eliminated redundant Vitest flag duplicates, preserving 100% CI parity with zero performance or build regression.
