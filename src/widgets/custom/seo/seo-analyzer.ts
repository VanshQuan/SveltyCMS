...
export class SeoAnalyzer {
  ...
  async analyze(
    ...
    const keywords = await this.analyzeKeywords(title, description, content);
    const readability = this.analyzeReadability(content);
    const contentStructure = this.analyzeContentStructure(content);
    const technical = this.analyzeTechnical(title, description, url);
  ...
  private async analyzeKeywords(
    ...
    const keywordOccurrences = (
      content.toLowerCase().match(new RegExp(keyword.replace(/\s+/g, "\\s+"), "g")) ?? []
    ).length;
    ...
    const firstOccurrencePercentage =
      firstOccurrence !== -1 ? (firstOccurrence / totalWords) * 100 : -1;
    ...
    return [
      {
        ...
        density,
        prominence: this.calculateKeywordProminence(
          inTitle,
          inDescription,
          firstOccurrencePercentage,
        ),
  ...
  private analyzeReadability(content: string): ReadabilityAnalysis {
    ...
    const complexSentences = sentences.filter(
      (sentence) => this.tokenize(sentence).length > 20,
    ).length;
  ...
  private analyzeContentStructure(content: string): ContentStructure {
    ...
    const externalLinks =
      ...
      internalLinks;
    const nofollowLinks = (content.match(/<a[^>]*rel=["'][^"']*nofollow[^"']*["'][^>]*>/gi) || [])
      .length;
  ...
  private analyzeTechnical(title: string, description: string, url?: string): TechnicalSeo {
    ...
    return {
      ...
      urlStructure: url
        ? {
            length: url.length,
            hasKeyword: this.config.focusKeyword
              ? url.toLowerCase().includes(this.config.focusKeyword.toLowerCase())
              : false,
  ...
  private calculateOverallScore(
    ...
    const readabilityScore = Math.max(
      0,
      Math.min(100, (readability.fleschKincaidScore / 100) * 100),
    );
    ...
    const overall = Math.round(
      (contentScore + technicalScore + readabilityScore + keywordsScore + socialScore) / 5,
    );
  ...
  private generateSuggestions(
    ...
    if (technical.titleLength < 30) {
      suggestions.push({
        ...
        description:
          "Your title is shorter than 30 characters. Consider adding more descriptive text.",
        impact: "high",
        effort: "easy",
        priority: 90,
        actionable: true,
    ...
    if (technical.descriptionLength > 160) {
      suggestions.push({
        id: "description-too-long",
        type: "error",
        category: "technical",
        title: "Meta description is too long",
        description: "Your meta description exceeds 160 characters and will be truncated.",
        impact: "high",
        effort: "easy",
        priority: 90,
        actionable: true,
    ...
    if (keywords.length > 0) {
      ...
      if (keyword.density > 2.5) {
        suggestions.push({
          ...
          description:
            "Your keyword density might be too high and could be seen as keyword stuffing.",
          impact: "medium",
          effort: "easy",
          priority: 65,
          actionable: true,
